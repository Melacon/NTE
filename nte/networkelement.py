...
import nte.emulator
from nte.utils import addCoreDefaultValuesToNode, printErrorAndExit, addCoreDefaultStatusValuesToNode
from nte.interface import *
from nte.odlregistration import registerNeToOdl, registerNeToOdlNewVersion
import nte.ethCrossConnect as EthXConn
...
class NetworkElement:
    ...
    def __init__(self, neUuid, neId, interfaces, eth_x_conn = None, dockerType = None, ptpClock = None):
        ...
        self.emEnv = nte.emulator.Emulator()
        ...
        self.equipmentConnectorConfigXmlNode = None
        self.wireInterfaceConfigXmlNode = None
        ...
        self.equipmentConnectorStatusXmlNode = None
        self.supportedAlarmsXmlNode = None
        self.wireInterfaceStatusXmlNode = None
    ...
    def saveConfigXmlTemplates(self):
        ...
        equipmentConnectorNode = self.equipmentConfigXmlNode.find('core-model:connector', self.namespaces)
        self.equipmentConnectorConfigXmlNode = copy.deepcopy(equipmentConnectorNode)
        self.equipmentConfigXmlNode.remove(equipmentConnectorNode)

        wireInterfaceNode = self.configRootXmlNode.find('microwave-model:wire-interface-pac', self.namespaces)
        self.wireInterfaceConfigXmlNode = copy.deepcopy(wireInterfaceNode)
        self.configRootXmlNode.remove(wireInterfaceNode)
    ...
    def saveStatusXmlTemplates(self):
        ...
        equipmentConnectorNode = self.equipmentStatusXmlNode.find('connector')
        self.equipmentConnectorStatusXmlNode = copy.deepcopy(equipmentConnectorNode)
        self.equipmentStatusXmlNode.remove(equipmentConnectorNode)

        wireInterfaceStatusNode = self.statusRootXmlNode.find('wire-interface-pac')
        self.wireInterfaceStatusXmlNode = copy.deepcopy(wireInterfaceStatusNode)
        self.statusRootXmlNode.remove(wireInterfaceStatusNode)
    ...
    def buildEquipmentModelXml(self):
        ...
        manufacturerIdentifier.text = 'WTE Device'
        ...
        if self.emEnv.portBasedEmulation is True:
            ...
            if self.dockerType == 'JavaNetconfServer':
                ...
                if self.emEnv.netconfCandidateDatastore is False:
                    stringCmd = "docker create -it --privileged -p %s:%s:830 -p %s:%s:22 --name=%s openyumante" % \
                                (self.managementIPAddressString, self.netconfPortNumber,
                                 self.managementIPAddressString, self.sshPortNumber, self.dockerName)
                else:
                    stringCmd = "docker create -it --privileged -e DATASTORE='candidate' -p %s:%s:830 -p %s:%s:22 --name=%s openyumante" % \
                                (self.managementIPAddressString, self.netconfPortNumber,
                                self.managementIPAddressString, self.sshPortNumber, self.dockerName)
        else:
            ...
            if self.dockerType == 'JavaNetconfServer':
                ...
                if self.emEnv.netconfCandidateDatastore is False:
                    stringCmd = "docker create -it --privileged -p %s:%s:830 -p %s:%s:22 --name=%s --network=%s openyumante" % \
                            (self.managementIPAddressString, self.netconfPortNumber,
                             self.managementIPAddressString, self.sshPortNumber,
                             self.dockerName, self.networkName)
                else:
                    stringCmd = "docker create -it --privileged -e DATASTORE='candidate' -p %s:%s:830 -p %s:%s:22 --name=%s --network=%s openyumante" % \
                                (self.managementIPAddressString, self.netconfPortNumber,
                                 self.managementIPAddressString, self.sshPortNumber,
                                 self.dockerName, self.networkName)
